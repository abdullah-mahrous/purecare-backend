...
import { listEquipment, getEquipment, createEquipment, updateEquipment, deleteEquipment } from "./equipments.controller";

const router = Router();
const equipmentUpload = memoryUpload();
router.get("/", validation(equipmentPaginationSchema, "query"), listEquipment);
router.get("/:id", validation(idSchema, "params"), getEquipment);
